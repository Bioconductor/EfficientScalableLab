...
Authors: Valerie Obenchain (<a
  href="mailto:vobencha@fredhutch.org">vobencha@fredhutch.org</a>), 
  Martin Morgan (<a
...
fl <- system.file("extdata", "hg19_alias.tab", package="BioC2015LargeData")
...
filename <- "data/vobencha/LargeData/srarchive/SRR1039508_sorted.bam"
...
`r Biocpkg("BiocParallel")` provides a standardized interface for 
simple parallel evaluation. The package builds provides access to the
`snow` and `multicore` functionality in the `parallel` package as well
as `BatchJobs` for running cluster jobs.
...
Full details on error handling, logging and debugging are in the BiocParallel
[Errors, Logs and
...
```{r co-bams}
fls <- dir("data/vobencha/LargeData/copynumber", ".bam$", full=TRUE)
